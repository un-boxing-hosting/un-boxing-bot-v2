...
module.exports = {
  name: 'enable',
  //aliases: ['', ''],
  description: 'enable s logging for events',
  cooldown: 5,
  usage: 'you need to specify a number with the event u want to log. type \`${prefix}help\`',
  guildOnly: true,
  args: true,
  async execute(bot, message, args) {
    ...
    switch (args[0]) {
      ...
      case "12":
        ...
        break;
      case "13":
        await db.set(`roleupdate_${message.guild.id}`, 'enabled')
        message.channel.send(`ok, enabled the logging for role update`)
        await db.delete(`allenabled_${message.guild.id}`)
        break;
      case "14":
        await db.set(`messageUpdate_${message.guild.id}`, 'enabled')
